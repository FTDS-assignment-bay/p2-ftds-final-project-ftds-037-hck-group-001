...
def render_prediction():
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            ...
            debt_ratio = st.number_input("Debt Ratio", min_value=0.0, value=0.8, step=0.01)
            income = st.number_input("Monthly Income", min_value=0.0, value=5000.0, step=100.0)
            open_loans = st.number_input("Open Credit Lines and Loans", min_value=0, value=8, step=1)

        with col2:
            real_estate_loans = st.number_input("Real Estate Loans", min_value=0, value=1, step=1)
            dependents = st.number_input("Dependents", min_value=0.0, value=1.0, step=1.0)
